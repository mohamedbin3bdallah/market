...
if(isset($_POST['pcode']))
{
	...
	include("../libs/config.php");
	include("../libs/opendb.php");	
	$result0 = $dbh->query("select count(*) as count from backs where product = '$pcode'");
	$row0 = $result0->fetch();
	if($row0['count'] != 0)
	{
		?>
		...
		<?php		
		$result = $dbh->query("select * from backs where product = '$pcode' order by time DESC , invoice ASC, size ASC");
		if(!empty($result))
		{		
...
if(isset($_POST['invoice']))
{
	...
	include("../libs/config.php");
	include("../libs/opendb.php");	
	$result0 = $dbh->query("select count(*) as count from backs where invoice = '$invoice'");
	$row0 = $result0->fetch();
	if($row0['count'] != 0)
	{
		?>
		...
		<?php
		$result = $dbh->query("select * from backs where invoice = '$invoice' order by time DESC , product ASC , size ASC");
		if(!empty($result))
		{		
			for($i=0; $row = $result->fetch(); $i++)
			{
